...
if [ -f "$CUR_DIR/main.go" ]; then
    ...
    go build -o "$CUR_DIR/blog_app" "$CUR_DIR/main.go" "$CUR_DIR/skin.go"
    echo "前台服务编译成功"
...
if [ -f "$CUR_DIR/admin.go" ]; then
    ...
    go build -o "$CUR_DIR/admin_app" "$CUR_DIR/admin.go" "$CUR_DIR/admin_helpers.go" "$CUR_DIR/admin_storage.go" "$CUR_DIR/skin.go"
    echo "后台服务编译成功"
